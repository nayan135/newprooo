document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (shortId) {
        try {
            const response = await fetch(`/.netlify/functions/get-name?shortId=${shortId}`);
    ...
    responseForm.addEventListener('click', async function(event) {
        if (event.target.classList.contains('response-button')) {
            ...
            try {
                await fetch('/.netlify/functions/submit-response', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({ shortId, response })
                });
                alert('Response sent!');
            } catch (err) {
